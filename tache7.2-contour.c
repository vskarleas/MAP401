...
Bezier3 approx_bezier3(Contour c, int j1, int j2)
{
    ...
    else if (n>2)
    {
        ...
        //Calcul a et b
        double a, b, lambda;
        a = (-15*n_double*n_double*n_double+5*n_double*n_double+2*n_double)/(3*(n_double+2)*(3*n_double*n_double+1));
        ...
        for (int i = j1; i <j2; i++)
        {
            id = T.tab[i];
            i_dbl= (double)i;
            alpha= (6*i_dbl*i_dbl*i_dbl*i_dbl) - (8*n_double*i_dbl*i_dbl) + (6*i_dbl*i_dbl) - (4*n_double*i_dbl) + (n_double*n_double*n_double*n_double) - (n_double*n_double);
